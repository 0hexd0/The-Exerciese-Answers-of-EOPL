  ...
  (lambda (proc1 vals)
    (cases proc proc1
      (procedure (vars body saved-env)
                 (value-of body (extend-env-by-two-lst vars vals saved-env))))))
          ...
          (cond
            ...
            ((eqv? cur 'call)
             (if
              ...
              (cons
                 (call-exp (parse-single(cadr exp)) (map parse-single (caddr exp)))
                 (replace (cdddr exp)))
              (cons cur (cons (cadr exp) (replace (cddr exp))))))
            ((eqv? cur 'proc)
             (if
              (and (identifier-list? (cadr exp))
                   (is-not-rator? (caddr exp)))
              (cons
                 (proc-exp (cadr exp)
                           (parse-single (caddr exp)))
...
(run '(let add1 proc (a b c) + a + b c call add1 (1 2 3)))